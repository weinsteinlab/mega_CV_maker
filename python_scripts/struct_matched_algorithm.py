...
nhl_labels = np.load("nhl_labels.npy") # key for indices of the 23 hallucinogen trajectories (5HT_0, 5HT_1, etc)
hl_labels = np.load("hl_labels.npy") # key for non-hallucinogen trajectories (LSD_0, LSD_1, etc)
...
hall_ligs = ["LSD", "DOI", "DMT", "MES"] # hallucinogen systems
nhall_ligs = ["5HT", "ERG", "LIS", "KET"] # non-hallucinogen systems
...
min_hall_prob = 0.6 # minimum state probability for hallucinogens. Each ligand-system should sample at least this fraction
first_frame=0      # option to skip some number of frames
...
# now apply filter on data gathered above to satisfy probability criteria. Searching for CVs that have values sampled by the non-hallucinogens less than X% of the time while these values are sampled by each hallucinogen system greater than y% of the time, on either side of the non-hallucinogen values
    ...
    for x in HL:          # loop over ligands
        ...
        match_ids = [i for i,x in enumerate(NHL[:,0,0]) if x in matches]

        for i,q in enumerate(x):  # loop over the thresholds
            nhl_max = NHL[match_ids,i,1].astype(np.float).max()   # maximum fraction sampled by the NHL systems at that threshold
            if np.logical_and(q[1].astype(np.float)>min_hall_prob,nhl_max<max_nhall_prob): 
                hit=True
                thresh = np.linspace(minn,maxx,16)[i]
            nhl_max = (1-NHL[match_ids,i,1].astype(np.float)).max()   # using 1-fraction to get fraction sampled below threshold. Want to allow Halls to sample on either side of Nhall values (bigger or smaller distances than Nhalls)
            if np.logical_and((1-q[1].astype(np.float))>min_hall_prob,nhl_max<max_nhall_prob): 
                hit=True
                thresh = np.linspace(minn,maxx,16)[i]
        hits.append((name, hit, thresh))
    
    if sum([x[1] for x in hits])==4: # If all 4 Hallucinogens pass criteria
